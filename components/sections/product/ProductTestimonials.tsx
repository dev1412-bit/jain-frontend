import type { Testimonial } from "@/store/cmsStore";
 ...
 return (
    <div className="py-8 border-t border-border">
      <div className="flex items-center justify-between mb-6">
        ...
        <Link href="#" className="text-xs text-brand hover:underline">View More</Link>
      </div>
        ...
        {displayTestimonials.map((t: Testimonial) => (
          <div key={t.id} className="bg-background border border-border rounded-2xl p-5 space-y-3">
            <div className="flex items-center gap-0.5">
              {[...Array(5)].map((_, i) => (
                <Star key={i} className={`h-3.5 w-3.5 ${i < t.rating ? "fill-brand text-brand" : "text-border"}`} />
              ))}
            </div>
            <p className="text-xs text-muted-foreground leading-relaxed line-clamp-4">
              &ldquo;{t.review}&rdquo; {/* ← back to review */}
            </p>
