...
export default function SignUpModal() {
  const { signUpOpen, closeSignUp, switchToSignIn } = useUIStore();
...
const onSubmit = async (data: SignUpForm) => {
  ...
  try {
   ...
   setAuth(res.user, res.user.role as Role); 
    closeSignUp();
    reset();

    toast.success("Account created!", {
      description: `Welcome to JainSoftware, ${res.user.name}!`,
    });
        ...
        <div className="bg-brand/5 border-b border-border px-6 pt-6 pb-5">
          <DialogHeader>
            ...
            <DialogDescription className="text-center text-sm text-muted-foreground">
              Get started with JainSoftware today
            </DialogDescription>
          </DialogHeader>
